 ...
 * ============================================================================ */
#include "ns3/greenWindowServer-helper.h"
#include "ns3/greenWindowClient.h"
#include "ns3/greenWindowClient-helper.h"
...
using namespace ns3;
NS_LOG_COMPONENT_DEFINE ("v2i-greenWindow-80211p");
...
main (int argc, char *argv[])
{
  ...
  if (verbose)
    {
      LogComponentEnable ("v2i-greenWindow-80211p", LOG_LEVEL_INFO);
      LogComponentEnable ("CABasicService", LOG_LEVEL_INFO);
      LogComponentEnable ("TLMService", LOG_LEVEL_INFO);
      LogComponentEnable ("greenWindowServer", LOG_LEVEL_INFO);
    }
  ...
  wifiPhy.EnablePcap ("v2i-greenWindow-80211p", netDevices);
  wifiPhy.EnablePcap ("v2i-greenWindow-80211p-RSU", netRSUs);
  ...
  greenWindowServerHelper greenWindowServerHelper_;
  greenWindowServerHelper_.SetAttribute ("Model", StringValue ("80211p"));
  greenWindowServerHelper_.SetAttribute ("Client", (PointerValue) sumoClient);
  greenWindowServerHelper_.SetAttribute ("RealTime", BooleanValue (realtime));
  greenWindowServerHelper_.SetAttribute ("AggregateOutput", BooleanValue (aggregate_out));
  greenWindowServerHelper_.SetAttribute ("CSV", StringValue (csv_name));
  greenWindowServerHelper_.SetAttribute ("MetricSupervisor", PointerValue (metSup));
  greenWindowServerHelper_.SetAttribute ("SendSPATEM", BooleanValue (send_spatem));
  ...
  for (auto rsu : rsuData)
    {
      ...
      ApplicationContainer AppServer = greenWindowServerHelper_.Install (rsuNode);
  ...
  greenWindowClientHelper greenWindowClientHelper_;
  Ipv4Address remoteHostAddr;

  greenWindowClientHelper_.SetAttribute ("Model", StringValue ("80211p"));
  greenWindowClientHelper_.SetAttribute ("ServerAddr", Ipv4AddressValue (remoteHostAddr));
  greenWindowClientHelper_.SetAttribute (
      "Client",
      (PointerValue) sumoClient); 
  greenWindowClientHelper_.SetAttribute ("PrintSummary", BooleanValue (print_summary));
  greenWindowClientHelper_.SetAttribute ("RealTime", BooleanValue (realtime));
  greenWindowClientHelper_.SetAttribute ("CSV", StringValue (csv_name));
  greenWindowClientHelper_.SetAttribute ("SendCAM", BooleanValue (send_cam));
  greenWindowClientHelper_.SetAttribute ("MetricSupervisor", PointerValue (metSup));
  ...
  STARTUP_FCN setupNewWifiNode = [&] (std::string vehicleID,
    ...
    ApplicationContainer ClientApp = greenWindowClientHelper_.Install (includedNode);
  ...
  SHUTDOWN_FCN shutdownWifiNode = [] (Ptr<Node> exNode, std::string vehicleID) {
    ...
    Ptr<greenWindowClient> greenWindowClient_ = exNode->GetApplication (0)->GetObject<greenWindowClient> ();
    if (greenWindowClient_)
      greenWindowClient_->StopApplicationNow ();
