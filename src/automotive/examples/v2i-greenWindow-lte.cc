...
using namespace ns3;
NS_LOG_COMPONENT_DEFINE ("v2i-greenWindow-LTE");
...
main (int argc, char *argv[])
{
  ...
  if (verbose)
    {
      LogComponentEnable ("v2i-greenWindow-LTE", LOG_LEVEL_INFO);
  ...
  greenWindowServerHelper greenWindowServHelper;
  greenWindowServHelper.SetAttribute ("Model", StringValue ("lte"));
  greenWindowServHelper.SetAttribute ("Client", (PointerValue) sumoClient);
  greenWindowServHelper.SetAttribute ("RealTime", BooleanValue (realtime));
  greenWindowServHelper.SetAttribute ("AggregateOutput", BooleanValue (aggregate_out));
  greenWindowServHelper.SetAttribute ("CSV", StringValue (csv_name));
  greenWindowServHelper.SetAttribute ("MetricSupervisor", PointerValue (metSup));
  greenWindowServHelper.SetAttribute ("SendSPATEM", BooleanValue (send_spatem));
  ...
  ApplicationContainer AppServer = greenWindowServHelper.Install (remoteHostContainer.Get (0));
  AppServer.Start (Seconds (0.0));
  AppServer.Stop (simulationTime - Seconds (0.1));

  
  greenWindowClientHelper greenWindowCliHelper;
  greenWindowCliHelper.SetAttribute ("Model", StringValue ("lte"));
  greenWindowCliHelper.SetAttribute ("ServerAddr", Ipv4AddressValue (remoteHostAddr));
  greenWindowCliHelper.SetAttribute (
      "Client",
      (PointerValue) sumoClient); 
  greenWindowCliHelper.SetAttribute ("PrintSummary", BooleanValue (print_summary));
  greenWindowCliHelper.SetAttribute ("RealTime", BooleanValue (realtime));
  greenWindowCliHelper.SetAttribute ("CSV", StringValue (csv_name));
  greenWindowCliHelper.SetAttribute ("SendCAM", BooleanValue (send_cam));
  greenWindowCliHelper.SetAttribute ("MetricSupervisor", PointerValue (metSup));
  ...
  STARTUP_FCN setupNewWifiNode = [&] (std::string vehicleID,
    ...
    ApplicationContainer ClientApp = greenWindowCliHelper.Install (includedNode);
