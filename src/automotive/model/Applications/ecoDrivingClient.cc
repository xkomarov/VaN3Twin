...
namespace ns3 {
NS_LOG_COMPONENT_DEFINE ("ecoDrivingClient");

NS_OBJECT_ENSURE_REGISTERED (ecoDrivingClient);

TypeId
ecoDrivingClient::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::ecoDrivingClient")
          .SetParent<Application> ()
          .SetGroupName ("Applications")
          .AddConstructor<ecoDrivingClient> ()
          .AddAttribute ("Model", "Access Technology Model (80211p or lte)", StringValue ("lte"),
                         MakeStringAccessor (&ecoDrivingClient::m_model), MakeStringChecker ())
          .AddAttribute (
              "PrintSummary", "To print summary at the end of simulation", BooleanValue (false),
              MakeBooleanAccessor (&ecoDrivingClient::m_print_summary), MakeBooleanChecker ())
          .AddAttribute ("RealTime", "To compute properly timestamps", BooleanValue (false),
                         MakeBooleanAccessor (&ecoDrivingClient::m_real_time),
                         MakeBooleanChecker ())
          .AddAttribute ("CSV", "CSV log name", StringValue (),
                         MakeStringAccessor (&ecoDrivingClient::m_csv_name), MakeStringChecker ())
          .AddAttribute ("ServerAddr", "Ip Addr of the server", Ipv4AddressValue ("10.0.0.1"),
                         MakeIpv4AddressAccessor (&ecoDrivingClient::m_server_addr),
                         MakeIpv4AddressChecker ())
          .AddAttribute ("Client", "TraCI client for SUMO", PointerValue (0),
                         MakePointerAccessor (&ecoDrivingClient::m_client),
                         MakePointerChecker<TraciClient> ())
          .AddAttribute (
              "MetricSupervisor",
              "Metric Supervisor to compute metrics according to 3GPP TR36.885 V14.0.0 page 70",
              PointerValue (0), MakePointerAccessor (&ecoDrivingClient::m_metric_supervisor),
              MakePointerChecker<MetricSupervisor> ())
          .AddAttribute ("SendCAM", "To enable/disable the transmission of CAM messages",
                         BooleanValue (true), MakeBooleanAccessor (&ecoDrivingClient::m_send_cam),
                         MakeBooleanChecker ());
  return tid;
}

ecoDrivingClient::ecoDrivingClient ()
{
...
void
ecoDrivingClient::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
...
void
ecoDrivingClient::StartApplication (void)
{
  NS_LOG_FUNCTION (this);
  ...
  m_caService.addCARxCallback (std::bind (&ecoDrivingClient::receiveCAM, this,
  ...
  m_tlmService.addTLMRxCallback (std::bind (&ecoDrivingClient::receiveSPATEM, this,
                                            std::placeholders::_1, std::placeholders::_2));
...
void
ecoDrivingClient::StopApplication ()
{
  NS_LOG_FUNCTION (this);
  Simulator::Cancel (m_sendCamEvent);
  Simulator::Cancel (m_spatemOut);
  Simulator::Cancel (m_ecoDrivingUpdateEvent);

  if (m_ecoDrivingActive)
    {
      try
        {
...
void
ecoDrivingClient::StopApplicationNow ()
{
  NS_LOG_FUNCTION (this);
  StopApplication ();
}
...
void
ecoDrivingClient::receiveCAM (asn1cpp::Seq<CAM> cam, Address from)
{
  (void) cam;
  (void) from;
}
...
void
ecoDrivingClient::receiveSPATEM (asn1cpp::Seq<SPATEM> spatem, Address from)
{
  Simulator::Cancel (m_spatemOut);
  m_spatem_received++;
  ...
  if (m_ecoDrivingUpdateEvent.IsExpired ())
    {
      m_ecoDrivingUpdateEvent =
          Simulator::Schedule (MilliSeconds (200), &ecoDrivingClient::updateglosaControl, this);
    }

  m_last_spatem_rx_time_ms = Simulator::Now ().GetMilliSeconds ();

  m_spatemOut = Simulator::Schedule (Seconds (2.0), &ecoDrivingClient::spatemOut, this);
}

void
ecoDrivingClient::applyGlosaAction (double speed, int r, int g, int b)
{
  m_client->TraCIAPI::vehicle.setSpeedMode (m_id, 7);
  m_client->TraCIAPI::vehicle.setSpeed (m_id, speed);
  m_ecoDrivingActive = true;
...
void
ecoDrivingClient::updateglosaControl (void)
{
  ...
  const double comfortDecel = 2.5;
  const double ecoDriving_MIN_DIST = 10.0;
  const double ecoDriving_MAX_DIST = 400.0;
  ...
  if (dist < ecoDriving_MIN_DIST || dist > ecoDriving_MAX_DIST)
    {
      if (m_ecoDrivingActive)
        {
          m_client->TraCIAPI::vehicle.setSpeedMode (m_id, 31);
      ...
      if (canMakeIt)
        {
          ...
          if (std::abs (targetSpeed - currentSpeed) < 1.0)
            {
              applyGlosaAction (targetSpeed, 0, 255, 0);
            }
          else if (targetSpeed > currentSpeed)
            {
              applyGlosaAction (targetSpeed, 50, 205, 50);
            }
          else
            {
              applyGlosaAction (targetSpeed, 173, 255, 47);
            }
        }
      else
        {
...
void
ecoDrivingClient::populateStaticTLData (void)
{
  auto tlsIDs = m_client->TraCIAPI::trafficlights.getIDList ();
