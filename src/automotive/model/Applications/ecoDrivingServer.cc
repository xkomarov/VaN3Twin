...
namespace ns3 {
NS_LOG_COMPONENT_DEFINE ("ecoDrivingServer");

NS_OBJECT_ENSURE_REGISTERED (ecoDrivingServer);

TypeId
ecoDrivingServer::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::ecoDrivingServer")
          .SetParent<Application> ()
          .SetGroupName ("Applications")
          .AddConstructor<ecoDrivingServer> ()
          .AddAttribute ("Model", "Access Technology Model (80211p or lte)", StringValue ("lte"),
                         MakeStringAccessor (&ecoDrivingServer::m_model), MakeStringChecker ())
          .AddAttribute ("AggregateOutput",
                         "If it is true, the server will print every second an aggregate output "
                         "about cam",
                         BooleanValue (false),
                         MakeBooleanAccessor (&ecoDrivingServer::m_aggregate_output),
                         MakeBooleanChecker ())
          .AddAttribute ("RealTime", "To compute properly timestamps", BooleanValue (false),
                         MakeBooleanAccessor (&ecoDrivingServer::m_real_time), MakeBooleanChecker ())
          .AddAttribute ("CSV", "CSV log name", StringValue (),
                         MakeStringAccessor (&ecoDrivingServer::m_csv_name), MakeStringChecker ())
          .AddAttribute ("Client", "TraCI client for SUMO", PointerValue (0),
                         MakePointerAccessor (&ecoDrivingServer::m_client),
                         MakePointerChecker<TraciClient> ())
          .AddAttribute (
              "MetricSupervisor",
              "Metric Supervisor to compute metric according to 3GPP TR36.885 V14.0.0 page 70",
              PointerValue (0), MakePointerAccessor (&ecoDrivingServer::m_metric_supervisor),
              MakePointerChecker<MetricSupervisor> ())
          .AddAttribute ("SendCAM", "To enable/disable the transmission of CAM messages",
                         BooleanValue (true), MakeBooleanAccessor (&ecoDrivingServer::m_send_cam),
                         MakeBooleanChecker ())
          .AddAttribute ("SendSPATEM", "To enable/disable the transmission of SPATEM messages",
                         BooleanValue (true), MakeBooleanAccessor (&ecoDrivingServer::m_send_spatem),
                         MakeBooleanChecker ())
          .AddAttribute ("SumoId", "SUMO ID of the POI/RSU this server represents",
                         StringValue ("poi_0"), MakeStringAccessor (&ecoDrivingServer::m_sumo_id),
                         MakeStringChecker ());

  return tid;
}

ecoDrivingServer::ecoDrivingServer ()
{
...
void
ecoDrivingServer::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
...
void
ecoDrivingServer::StartApplication (void)
{
  NS_LOG_FUNCTION (this);
  ...
  m_caService.addCARxCallback (
      std::bind (&ecoDrivingServer::receiveCAM, this, std::placeholders::_1, std::placeholders::_2));
  ...
  if (m_aggregate_output)
    m_aggegateOutputEvent =
        Simulator::Schedule (Seconds (1), &ecoDrivingServer::aggregateOutput, this);
}

void
ecoDrivingServer::StopApplication ()
{
  NS_LOG_FUNCTION (this);
...
void
ecoDrivingServer::StopApplicationNow ()
{
  NS_LOG_FUNCTION (this);
  StopApplication ();
}
...
void
ecoDrivingServer::receiveCAM (asn1cpp::Seq<CAM> cam, Address from)
{
  m_cam_received++;
...
void
ecoDrivingServer::aggregateOutput ()
{
  std::cout << Simulator::Now () << "," << m_cam_received << std::endl;
  m_aggegateOutputEvent = Simulator::Schedule (Seconds (1), &ecoDrivingServer::aggregateOutput, this);
}
