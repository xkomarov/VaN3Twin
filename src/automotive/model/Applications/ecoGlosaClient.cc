  ...
  static TypeId tid =
      TypeId ("ns3::ecoGlosaClient")
          ...
          .AddAttribute ("Model", "Access Technology Model (80211p or lte)", StringValue ("lte"),
                         MakeStringAccessor (&ecoGlosaClient::m_model), MakeStringChecker ())
          .AddAttribute (
              "PrintSummary", "To print summary at the end of simulation", BooleanValue (false),
              MakeBooleanAccessor (&ecoGlosaClient::m_print_summary), MakeBooleanChecker ())
          .AddAttribute ("RealTime", "To compute properly timestamps", BooleanValue (false),
                         MakeBooleanAccessor (&ecoGlosaClient::m_real_time), MakeBooleanChecker ())
          .AddAttribute ("CSV", "CSV log name", StringValue (),
                         MakeStringAccessor (&ecoGlosaClient::m_csv_name), MakeStringChecker ())
          ...
          .AddAttribute (
              ...
              PointerValue (0), MakePointerAccessor (&ecoGlosaClient::m_metric_supervisor),
              MakePointerChecker<MetricSupervisor> ())
          .AddAttribute ("SendCAM", "To enable/disable the transmission of CAM messages",
                         BooleanValue (true), MakeBooleanAccessor (&ecoGlosaClient::m_send_cam),
                         MakeBooleanChecker ());
  return tid;
}
...
ecoGlosaClient::StartApplication (void)
{
  ...
  m_caService.addCARxCallback (
      std::bind (&ecoGlosaClient::receiveCAM, this, std::placeholders::_1, std::placeholders::_2));
  ...
  m_tlmService.addTLMRxCallback (std::bind (&ecoGlosaClient::receiveSPATEM, this,
...
ecoGlosaClient::updateglosaControl (void)
{
  ...
  const double ecoGlosa_MAX_DIST = 400.0; // m — too far, timing unreliable
