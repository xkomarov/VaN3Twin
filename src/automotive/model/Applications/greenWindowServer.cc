...
namespace ns3 {
NS_LOG_COMPONENT_DEFINE ("greenWindowServer");

NS_OBJECT_ENSURE_REGISTERED (greenWindowServer);

TypeId
greenWindowServer::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::greenWindowServer")
          .SetParent<Application> ()
          .SetGroupName ("Applications")
          .AddConstructor<greenWindowServer> ()
          .AddAttribute ("Model", "Access Technology Model (80211p or lte)", StringValue ("lte"),
                         MakeStringAccessor (&greenWindowServer::m_model), MakeStringChecker ())
          .AddAttribute ("AggregateOutput",
                         "If it is true, the server will print every second an aggregate output "
                         "about cam",
                         BooleanValue (false),
                         MakeBooleanAccessor (&greenWindowServer::m_aggregate_output),
                         MakeBooleanChecker ())
          .AddAttribute ("RealTime", "To compute properly timestamps", BooleanValue (false),
                         MakeBooleanAccessor (&greenWindowServer::m_real_time), MakeBooleanChecker ())
          .AddAttribute ("CSV", "CSV log name", StringValue (),
                         MakeStringAccessor (&greenWindowServer::m_csv_name), MakeStringChecker ())
          .AddAttribute ("Client", "TraCI client for SUMO", PointerValue (0),
                         MakePointerAccessor (&greenWindowServer::m_client),
                         MakePointerChecker<TraciClient> ())
          .AddAttribute (
              "MetricSupervisor",
              "Metric Supervisor to compute metric according to 3GPP TR36.885 V14.0.0 page 70",
              PointerValue (0), MakePointerAccessor (&greenWindowServer::m_metric_supervisor),
              MakePointerChecker<MetricSupervisor> ())
          .AddAttribute ("SendCAM", "To enable/disable the transmission of CAM messages",
                         BooleanValue (true), MakeBooleanAccessor (&greenWindowServer::m_send_cam),
                         MakeBooleanChecker ())
          .AddAttribute ("SendSPATEM", "To enable/disable the transmission of SPATEM messages",
                         BooleanValue (true), MakeBooleanAccessor (&greenWindowServer::m_send_spatem),
                         MakeBooleanChecker ())
          .AddAttribute ("SumoId", "SUMO ID of the POI/RSU this server represents",
                         StringValue ("poi_0"), MakeStringAccessor (&greenWindowServer::m_sumo_id),
                         MakeStringChecker ());

  return tid;
}

greenWindowServer::greenWindowServer ()
{
...
void
greenWindowServer::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
...
void
greenWindowServer::StartApplication (void)
{
  NS_LOG_FUNCTION (this);
  ...
  m_caService.addCARxCallback (
      std::bind (&greenWindowServer::receiveCAM, this, std::placeholders::_1, std::placeholders::_2));
  ...
  if (m_aggregate_output)
    m_aggegateOutputEvent = Simulator::Schedule (Seconds (1), &greenWindowServer::aggregateOutput, this);
}

void
greenWindowServer::StopApplication ()
{
  NS_LOG_FUNCTION (this);
...
void
greenWindowServer::StopApplicationNow ()
{
  NS_LOG_FUNCTION (this);
  StopApplication ();
}
...
void
greenWindowServer::receiveCAM (asn1cpp::Seq<CAM> cam, Address from)
{
  m_cam_received++;
...
void
greenWindowServer::aggregateOutput ()
{
  std::cout << Simulator::Now () << "," << m_cam_received << std::endl;
  m_aggegateOutputEvent = Simulator::Schedule (Seconds (1), &greenWindowServer::aggregateOutput, this);
}
