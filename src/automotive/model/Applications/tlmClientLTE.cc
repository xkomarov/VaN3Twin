  ...
  static TypeId tid =
      TypeId ("ns3::tlmClientLTE")
          ...
          .AddAttribute ("PrintSummary", "To print summary at the end of simulation",
                         BooleanValue (false), MakeBooleanAccessor (&tlmClientLTE::m_print_summary),
                         MakeBooleanChecker ())
          .AddAttribute ("RealTime", "To compute properly timestamps", BooleanValue (false),
                         MakeBooleanAccessor (&tlmClientLTE::m_real_time), MakeBooleanChecker ())
          .AddAttribute ("CSV", "CSV log name", StringValue (),
                         MakeStringAccessor (&tlmClientLTE::m_csv_name), MakeStringChecker ())
          ...
          .AddAttribute (
              ...
              PointerValue (0), MakePointerAccessor (&tlmClientLTE::m_metric_supervisor),
              MakePointerChecker<MetricSupervisor> ())
          .AddAttribute ("SendCAM", "To enable/disable the transmission of CAM messages",
                         BooleanValue (true), MakeBooleanAccessor (&tlmClientLTE::m_send_cam),
                         MakeBooleanChecker ());
  return tid;
}
...
tlmClientLTE::StartApplication (void)
{
  ...
  m_caService.addCARxCallback (
      std::bind (&tlmClientLTE::receiveCAM, this, std::placeholders::_1, std::placeholders::_2));
...
tlmClientLTE::receiveSPATEM (asn1cpp::Seq<SPATEM> spatem, Address from)
{
  ...
  if (m_glosaUpdateEvent.IsExpired ())
    {
      m_glosaUpdateEvent =
          Simulator::Schedule (MilliSeconds (200), &tlmClientLTE::updateGlosaControl, this);
    }
