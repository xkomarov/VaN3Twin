  ...
  static TypeId tid =
      TypeId ("ns3::tlmServer80211p")
          ...
          .AddAttribute ("RealTime", "To compute properly timestamps", BooleanValue (false),
                         MakeBooleanAccessor (&tlmServer80211p::m_real_time), MakeBooleanChecker ())
          .AddAttribute ("CSV", "CSV log name", StringValue (),
                         MakeStringAccessor (&tlmServer80211p::m_csv_name), MakeStringChecker ())
          ...
          .AddAttribute (
              ...
              PointerValue (0), MakePointerAccessor (&tlmServer80211p::m_metric_supervisor),
              MakePointerChecker<MetricSupervisor> ())
          .AddAttribute ("SendCAM", "To enable/disable the transmission of CAM messages",
                         BooleanValue (true), MakeBooleanAccessor (&tlmServer80211p::m_send_cam),
                         MakeBooleanChecker ())
          .AddAttribute ("SendSPATEM", "To enable/disable the transmission of SPATEM messages",
                         BooleanValue (true), MakeBooleanAccessor (&tlmServer80211p::m_send_spatem),
                         MakeBooleanChecker ());
...
tlmServer80211p::StartApplication (void)
{
  ...
  m_caService.addCARxCallback (
      std::bind (&tlmServer80211p::receiveCAM, this, std::placeholders::_1, std::placeholders::_2));
  ...
  // Автоматическое определение ближайшего светофора к данной RSU
  ...
  if (target_tls_id.empty ())
    {
      NS_LOG_WARN ("[" << m_id << "] No traffic lights found in SUMO!");
    }
  else
    {
      NS_LOG_INFO ("[" << m_id << "] Auto-detected TLS: " << target_tls_id
                       << " (distance: " << minDist << " m)");
    }

  uint16_t area_radius = 400; // GLOSA needs ~400m advance notice for speed adjustment [m]
  ...
  if (m_aggregate_output)
    m_aggegateOutputEvent =
        Simulator::Schedule (Seconds (1), &tlmServer80211p::aggregateOutput, this);
}
