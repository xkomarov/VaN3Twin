...
BSContainer::BSContainer (unsigned long fixed_stationid, long fixed_stationtype,
...
void
BSContainer::setVAMmetricsfile (std::string file_name, bool collect_metrics)
{
...
BSContainer::setupContainer (bool CABasicService_enabled, bool DENBasicService_enabled,
                             bool VRUBasicService_enabled, bool CPMBasicService_enabled,
                             bool TLMService_enabled)
{
  if (CABasicService_enabled == false && DENBasicService_enabled == false &&
      VRUBasicService_enabled == false && CPMBasicService_enabled == false &&
      TLMService_enabled == false)
    {
      NS_FATAL_ERROR ("Error. Called setupContainer() asking for enabling zero Basic Services. "
                      "Aborting simulation.");
    }

  if (m_socket == nullptr || m_mobility_client == nullptr || m_station_id == ULONG_MAX ||
      m_stationtype == LONG_MAX)
    {
      NS_FATAL_ERROR ("Error. Attempted to call setupContainer() on an uninitialized BSContainer. "
                      "Aborting simulation.");
    }
  ...
  if (m_stationtype == StationType_pedestrian)
    {
      m_vrudp_ptr =
          new VRUdp (m_mobility_client, m_sumo_pedid_prefix + std::to_string (m_station_id));
  ...
  if (CPMBasicService_enabled == true)
    {
      ...
      m_cpbs.addCPRxCallback (
          std::bind (&BSContainer::receiveCPM, this, std::placeholders::_1, std::placeholders::_2));
...
BSContainer::setDENMCircularGeoArea (double lat, double lon, int radius_meters)
{
  if (m_is_configured == false)
    {
      NS_FATAL_ERROR ("Error. setDENMCircularGeoArea() should not be called before "
                      "setupContainer(). Please adjust your code.");
    }
          ...
          for (int j = 0; j < PObjects_size; j++)
            {
              ...
              if (m_recvCPMmap[fromID].find (asn1cpp::getField (PO_seq->objectId, long)) ==
                  m_recvCPMmap[fromID].end ())
                {
                  ...
                  if (m_LDM->lookup (asn1cpp::getField (PO_seq->objectId, long), PO_data) ==
                  ...
                  else
                    {
                      ...
                      //Update recvCPMmap
                      m_recvCPMmap[fromID][asn1cpp::getField (PO_seq->objectId, long)] =
                          asn1cpp::getField (PO_seq->objectId, long);
                    }
                }
              else
                {
                  // We have already receive this object from this vehicle
                  m_LDM->insert (BSContainer::translateCPMdata (
                      cpm, PO_seq, i,
                      m_recvCPMmap[fromID][asn1cpp::getField (PO_seq->objectId, long)]));
                }
            }
                               ...
                               asn1cpp::Seq<PerceivedObject> object, int objectIndex, int newID)
{
  ...
  if (m_mobility_client != nullptr)
    {
      libsumo::TraCIPosition traciPosition =
          m_mobility_client->TraCIAPI::simulation.convertLonLattoXY (fromLon, fromLat);
  ...
  if (m_mobility_client != nullptr)
    {
      libsumo::TraCIPosition traciPosition =
          m_mobility_client->TraCIAPI::simulation.convertXYtoLonLat (objectPosition.x,
                                                                     objectPosition.y);
