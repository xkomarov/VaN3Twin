...
public:
  BSContainer ();
  ~BSContainer ();
  BSContainer (unsigned long fixed_stationid, long fixed_stationtype,
               Ptr<TraciClient> mobility_client, bool real_time, Ptr<Socket> socket_rxtx);
  ...
  void
  setMobilityClient (Ptr<TraciClient> mobility_client)
  {
    m_mobility_client = mobility_client;
  }

  void
  linkMetricSupervisor (Ptr<MetricSupervisor> metSup)
  {
    m_metric_sup_ptr = metSup;
  }
  void
  enablePRRSupervisorForGNBeacons ()
  {
    m_prrsup_beacons = true;
  }
  void
  disablePRRSupervisorForGNBeacons ()
  {
    m_prrsup_beacons = false;
  }

  void
  addCAMRxCallback (
      std::function<void (asn1cpp::Seq<CAM>, Address, StationID_t, StationType_t, SignalInfo)>
          rx_callback)
  {
    m_CAReceiveCallbackExtended = rx_callback;
  }
  void
  addDENMRxCallback (
      std::function<void (denData, Address, unsigned long, long, SignalInfo)> rx_callback)
  {
    m_DENReceiveCallbackExtended = rx_callback;
  }
  void
  addVAMRxCallback (
      std::function<void (asn1cpp::Seq<VAM>, Address, StationID_t, StationType_t)> rx_callback)
  {
    m_VAMReceiveCallbackExtended = rx_callback;
  }
  void
  addCPMRxCallback (std::function<void (asn1cpp::Seq<CollectivePerceptionMessage>, Address,
                                        StationID_t, StationType_t, SignalInfo)>
                        rx_callback)
  {
    m_CPMReceiveCallbackExtended = rx_callback;
  }
  void
  addSPATEMRxCallback (
  ...
  void
  changeSUMO_ID_prefix (std::string new_prefix)
  {
    m_sumo_vehid_prefix = new_prefix;
  }

  void setupContainer (bool CABasicService_enabled, bool DENBasicService_enabled,
                       bool VRUBasicService_enabled, bool CPBasicService_enabled,
                       bool TLMService_enabled);
...
private:
  ...
  std::function<void (asn1cpp::Seq<CAM>, Address, StationID_t, StationType_t, SignalInfo)>
      m_CAReceiveCallbackExtended;
  std::function<void (denData, Address, unsigned long, long, SignalInfo)>
      m_DENReceiveCallbackExtended;
  std::function<void (asn1cpp::Seq<VAM>, Address, StationID_t, StationType_t)>
      m_VAMReceiveCallbackExtended;
  std::function<void (asn1cpp::Seq<CollectivePerceptionMessage>, Address, StationID_t,
                      StationType_t, SignalInfo)>
      m_CPMReceiveCallbackExtended;
  std::function<void (asn1cpp::Seq<SPATEM>, Address, StationID_t, StationType_t, SignalInfo)>
  ...
  std::map<int, std::map<int, int>>
      m_recvCPMmap; //! Structure mapping, for each CV that we have received a CPM from, the CPM's PO ids with the ego LDM's PO ids
};
} // namespace ns3
