  ...
  if (!m_isStatic)
    {
      ...
      m_vehicle_length =
          VDPValueConfidence<long, long> (m_traci_client->TraCIAPI::vehicle.getLength (m_id) * DECI,
...
VDPTraCI::getCAMMandatoryData ()
{
  ...
  /* Altitude [0,01 m] */
  CAMdata.altitude =
      VDPValueConfidence<> (AltitudeValue_unavailable, AltitudeConfidence_unavailable);
  ...
  if (!m_isStatic)
    CAMdata.longAcceleration =
        VDPValueConfidence<> (m_traci_client->TraCIAPI::vehicle.getAcceleration (m_id) * DECI,
                              AccelerationConfidence_unavailable);

  /* Heading WGS84 north [0.1 degree] */
  if (!m_isStatic)
    CAMdata.heading = VDPValueConfidence<> (
        m_traci_client->TraCIAPI::vehicle.getAngle (m_id) * DECI, HeadingConfidence_unavailable);
  ...
  CAMdata.curvature =
      VDPValueConfidence<> (CurvatureValue_unavailable, CurvatureConfidence_unavailable);
...
VDPTraCI::getCPMMandatoryData ()
{
  ...
  /* Longitudinal acceleration [0.1 m/s^2] */
  if (!m_isStatic)
    CPMdata.longAcceleration =
        VDPValueConfidence<> (m_traci_client->TraCIAPI::vehicle.getAcceleration (m_id) * DECI,
                              AccelerationConfidence_unavailable);

  /* Heading WGS84 north [0.1 degree] */
  if (!m_isStatic)
    CPMdata.heading = VDPValueConfidence<> (
        m_traci_client->TraCIAPI::vehicle.getAngle (m_id) * DECI, HeadingConfidence_unavailable);
